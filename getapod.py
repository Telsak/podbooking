...
def set_booking(roomdata, epoch, pod, form):
    ...
    if epoch_hr(epoch) == 10:
        ...
        hr_mod = -3
    if hr_mod != 0:
        verify_booking = Bookings.query.filter(Bookings.time==epoch+(3600*hr_mod)).filter(Bookings.room==roomdata.id).filter(Bookings.pod==ord(pod.upper())-64).filter(Bookings.name1==current_user.username).all()
...
def book(room='Null', caldate='Null', hr='Null', pod='Null'):
    if request.method == 'POST' and int(hr) in BOOK_HOURS:
        ...
        if 'Null' in locals().values():
            return redirect(url_for("show", room="D119A", caldate=date_to_str()), code=302)
        else:
            roomdata = Rooms.query.filter(Rooms.name==room.upper()).all()[0]
...
def delete(room='Null', caldate='Null', hr='Null', pod='Null'):
    ...
    if 'Null' in locals().values():
        ...
        try:
            ...
            return redirect(url_for("show", room="D119A", code=302))
        fac='DELETE'
...
def index(data='Null'):
    ...
    if 'php' in data:
        ...
        try:
            ...
            if labroom in ['B112', 'B114', 'B118', 'B123', 'B125']:
                labroom = 'D119A'
